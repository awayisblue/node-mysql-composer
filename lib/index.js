...
var utils = {
    isSubAArrray:function(subArray,arr){
    ...
    return {
        ...
        insert:function(config,callback){
            ...
            if(!keysForUpdate || keysForUpdate.length===0){
                ...
                if(!utils.isSubAArrray(keysForUpdate,Object.keys(data))) throw new Error('onDuplicateKeyUpdate array elements not match keys in data')
                var updateData = {}
            ...
            var insertString = operation.composeInsertString(data);
            var sql = 'insert '+(ignore?'ignore':'')+' into '+tableName+' '+insertString.columnStr+' values '+insertString.valueStr+' '+updateClause;
